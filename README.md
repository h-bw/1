...
项目中包含一个轻量级推荐模块，当前实现为基于用户行为的协同过滤推荐，并保留热门商品作为冷启动补充。

当前实现思路大致为：

1. 读取所有用户历史订单，构建用户-商品交互矩阵
2. 统计当前用户的已购商品及偏好强度
3. 使用余弦相似度计算当前用户与其他用户的相似度
4. 对相似用户购买过而当前用户未购买的商品进行加权评分
5. 按照协同过滤得分排序生成推荐结果
6. 当用户没有历史订单或候选商品不足时，回退到热门商品推荐
